...
from system.flcore.clients.clientsed_trigger import clientSed_Trigger
...
class FedSed_Trigger(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        self.set_clients(clientSed_Trigger)
        ...
        self.result_dict_trigger = {self.client_to_dataset_name[index]: [] for index in range(args.num_clients)}  # 记录trigger测试结果
        self.result_dict_trigger_later = {self.client_to_dataset_name[index]: [] for index in range(args.num_clients)}  # 记录trigger测试结果
    ...
    def send_models(self, ground):
        ...
        if ground == 0 or not self.bays:
            ...
            for key, global_model in self.service_agg_dic.items():
                best_weights, best_score, iteration_records, optimizer, best_model = iterative_adjustment((self.clients[key].model, global_model), self.clients[key].g, self.args, self.clients[key].validation_indices)
                # 全局模型保持不变
    ...
    def getdata(self):
        ...
        nx_graph = nx.generators.random_graphs.erdos_renyi_graph(num_nodes, edge_prob, directed=False)
    ...
    def test(self, pos="before"):
        ...
        for client in self.clients:
            for time in range(5):
                nmi_poi, ami_poi, ari_poi, nmi_trigger, ami_trigger, ari_trigger = client.test_trigger()
                if pos == "before":
                    self.result_dict[self.client_to_dataset_name[client.id]].append([round(nmi_poi, 2), round(ami_poi, 2), round(ari_poi, 2)])
                    self.result_dict_trigger[self.client_to_dataset_name[client.id]].append([round(nmi_trigger, 2), round(ami_trigger, 2), round(ari_trigger, 2)])
                else:
                    self.result_dict_later[self.client_to_dataset_name[client.id]].append([round(nmi_poi, 2), round(ami_poi, 2), round(ari_poi, 2)])
                    self.result_dict_trigger_later[self.client_to_dataset_name[client.id]].append([round(nmi_trigger, 2), round(ami_trigger, 2), round(ari_trigger, 2)])
                if time == 0:
                    print(f"Client {client.id} - {self.client_to_dataset_name[client.id]} NMI: {round(nmi_poi, 2)}, AMI: {round(ami_poi, 2)}, ARI: {round(ari_poi, 2)},")
