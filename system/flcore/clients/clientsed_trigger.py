...
from system.utils.data_utils import extract_embeddings, evaluate


class clientSed_Trigger(Client):
        ...
        for epoch in range(self.args.max_local_epochs):
            ...
            for batch_id, (input_nodes, output_nodes, blocks) in enumerate(dataloader):
                ...
                if(ground != 0 and self.pair_loss):
                    ...
                    # 计算蒸馏损失的系数
                    alpha = torch.exp(-loss_diff).item()
                    alpha = min(alpha, 1)
                ...
                tepoch.set_postfix({"loss":loss.item(), "alpha":alpha if ground != 0 and self.pair_loss else 0})
                tepoch.update(1)
                # end one batch
    ...
    def test_trigger(self):
        ...
        nmi_poi, ami_poi, ari_poi, nmi_hdbcan, ami_hdbcan, ari_hdbcan = evaluate(extract_features, extract_labels,
                                                                     np.arange(0, len(self.test_indices), 1),
                                                                     self.num_isolated_nodes,
                                                                     self.embedding_save_path,
                                                                     True)

        nmi_trigger, ami_trigger, ari_trigger, nmi_hdbcan_trigger, ami_hdbcan_trigger, ari_hdbcan_trigger = evaluate(extract_features, extract_labels,
                                                                     np.arange(0, len(self.test_indices), 1)[(int)(len(self.test_indices) * self.args.poison_rate):],
                                                                     self.num_isolated_nodes,
                                                                     self.embedding_save_path,
                                                                     True)

        return nmi_poi, ami_poi, ari_poi, nmi_trigger, ami_trigger, ari_trigger
